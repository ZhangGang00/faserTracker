...
static double target_z[9]={0, 50, 100, 947, 997, 1047, 1898, 1948, 1998};
static int charge=1;
...
int main(int argc, char ** argv) {
    ...
    TFile *outf = new TFile("/afs/cern.ch/user/g/gang/FASER/faser_tracker/helix_2particle_output.root","recreate");
    ...
    double hit[N_plane][10]={0}; //hit[i][0] is the hits in corresponding layer, hit[i][j>0] are coordinates
    int hit_rowID[N_plane][5]={0};
    int first_record[N_plane]={-1}; // record the which space point used in the first track of mu+/mu-
    int N_hit_plane=0;
...
#if 1
        status=0;	
	int cal_stat=1, find_truth_2=0, find_truth_plus=0, find_truth_minus=0, first_flag=0;
	N_shared_sp=0;
	d_truth_hits.clear();
	Is_shared_sp.clear();
	...
	N_hit_plane=0;
	for (int i=0; i<N_plane; i++) { for (int j=0; j<10; j++) hit[i][j]=0; }
	for (int i=0; i<N_plane; i++) { for (int j=0; j<5; j++) hit_rowID[i][j]=0; }
	for (int i=0; i<N_plane; i++) { first_record[i]=-1; }
	...
	track_points_rowID.clear();
		  ...
		  if (std::fabs(outvect[0])>200 || std::fabs(outvect[1])>200) cal_stat=0;
	...
	while (hit[k][0] != 1) { k++; }  // k_th plane is the first plane with only one space point
        for (int i=0; i<N_plane; i++)  // keep the space point closest to h[k][] in each plane
	  {
	     int temp_sp=-9999;
	     if (hit[i][0] > 0) {
// find the space point closest to expected truth position on each plane		     
		...
		for (int j=1; j<hit[i][0]; j++) {
                   ...
                   if (std::pow(hit[i][2*j+1]-truth_x[i],2.0)+std::pow(hit[i][2*j+2]-truth_y[i],2.0) < std::pow(tempvec.X()-truth_x[i],2.0)+std::pow(tempvec.Y()-truth_y[i],2.0)) 
		    {
		     ...
		     temp_rowID=hit_rowID[i][j];
		     temp_sp=j;
		    }
		} 
#endif
		//for (int j=0; j<hit[i][0]; j++) { cout<<"plane = "<<i<<"   (x,y) = ("<<hit[i][2*j+1]<<", "<<hit[i][2*j+2]<<")"<<"\n"; }
		//cout<<"distance = "<<std::pow(tempvec.X()-truth_x[i],2.0)+std::pow(tempvec.Y()-truth_y[i],2.0)<<"\n";
                //if (std::sqrt(std::pow(tempvec.X()-truth_x[i],2.0)+std::pow(tempvec.Y()-truth_y[i],2.0)) > 5) continue;
		...
		if (first_flag==1 && temp_sp!=-9999) first_record[i]=temp_sp;
		if (first_flag==2) {
		  if (temp_sp == first_record[i]) { Is_shared_sp.push_back(1); N_shared_sp++; }
		    else Is_shared_sp.push_back(0); 
	           }
	  }
